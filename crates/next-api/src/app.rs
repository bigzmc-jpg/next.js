...
impl AppEndpoint {
    ...
    async fn output(self: Vc<Self>) -> Result<Vc<AppEndpointOutput>> {
        ...
        // Create and register the source map for the polyfill
        if let Some(generate_source_map) = ResolvedVc::try_sidecast::<
            Box<dyn turbopack_core::source_map::GenerateSourceMap>,
        >(polyfill_output_asset)
        {
            let polyfill_source_map_asset =
                SourceMapAsset::new_fixed(polyfill_output_path.clone(), *generate_source_map)
                    .to_resolved()
                    .await?;
